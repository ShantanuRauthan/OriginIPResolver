...
Examples:
  ...
  python main.py example.com --no-subenum --no-crtsh
  python main.py example.com --verbose
  python main.py example.com --no-ssl --no-subenum
        """,
    )
...
def main():
    ...
    try:
        ...
        print(f"[!] Error during origin IP resolution: {e}")
        sys.exit(1)

    try:
        osint_results = gather_osint(domain, ssl=not args.no_ssl)
    except Exception as e:
        print(f"[!] Error during OSINT gathering: {e}")
